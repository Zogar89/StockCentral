...
def test_maybe_update_provider_stock_history_appends_daily_capture_and_trims_to_30_days(tmp_path):
...
def test_maybe_update_provider_stock_history_replaces_same_day_capture(tmp_path):
    history_path = tmp_path / "provider_stock_history.json"
...
def test_maybe_update_provider_stock_history_skips_outside_snapshot_hour(tmp_path):
    history_path = tmp_path / "provider_stock_history.json"
    history_path.write_text(json.dumps({"days": []}), encoding="utf-8")
    payload = payload_with_sources(
    ...
    assert json.loads(history_path.read_text(encoding="utf-8")) == {"days": []}
